	...
	public SpawnMenu()
	{
		...
		var left = Add.Panel( "left" );
		{
...

			{
				...
				var vehicles = body.AddChild<VrpVehiclesList>();
				tabs.AddButtonActive( "Vehicles", ( b ) => vehicles.SetClass( "active", b ) );

				var items = body.AddChild<VrpItemsList>();
				tabs.AddButtonActive( "VRP Items", ( b ) => items.SetClass( "active", b ) );
			}
		}
	...
	public override void Tick()
	{
		...
		if ( Input.Down( InputButton.Menu ) )
		{
			if ( !_qDown )
			{
				_qDown = true;
				_open = !_open;
				Parent.SetClass( "spawnmenuopen", _open );
			}
		}
		else
		{
